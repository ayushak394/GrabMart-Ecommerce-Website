...
const Profile = ({ totalItems }) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await axios.post(
        // to upload file to server
        `http://localhost:4000/Profile/uploadProfilePic/${userId}`,
        formData,
        {
  ...
  const handleSubmitFeedback = async () => {
    ...
    try {
      await axios.post(`http://localhost:4000/Profile/submitFeedback`, {
        feedback,
      });
  ...
  useEffect(() => {
    const fetchProfilePic = async () => {
      try {
        const response = await axios.get(
          `http://localhost:4000/Profile/getProfilePic/${userId}`
        );
        setProfilePic(response.data.profilePic);
      } catch (error) {
    ...
    const fetchUsername = async () => {
      try {
        const response = await axios.get(
          `http://localhost:4000/Profile/getUsername/${userId}`
        );
    ...
    const fetchEmail = async () => {
      try {
        const response = await axios.get(
          `http://localhost:4000/Profile/getEmail/${userId}`
        );
    ...
    <>
      <div className="ProfilePage">
        ...
        <div className="feedback-section">
          <h1>We would love to here your feedback!</h1>
          <textarea
            value={feedback}
